...
- **Management, not restraint.** Coding often needs a lot of context, so nothing is capped by a count: every node Jev classifies active and every earlier turn it judges relevant goes in, whole. Jev's own reading limit (a trimmed copy of a long turn) never limits what the model receives. What remains open is the model's real context window: when relevant material exceeds it, the next step is to rank by relevance within that budget rather than fail.
