...
pub fn load(
...
) -> Result<Vec<Arc<dyn InferenceProvider>>, String> {
    ...
    for config in configs {
        ...
        providers.push(Arc::new(OpenAiCompatible::new(config)));
    }
    Ok(providers)
}
...
mod tests {
    ...
    fn the_fake_provider_is_always_available_and_presets_follow_their_key_variables() {
        ...
        let none = load(dir.path(), |_| None, |_| {}).unwrap();
        ...
        assert_eq!(ids(&with_key.unwrap()), ["fake", "openrouter"]);
    }
